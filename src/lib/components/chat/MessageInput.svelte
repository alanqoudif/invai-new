			...
			>
				<div class="">
					...
					{:else}
						...
						>
							<div
			class="flex-1 flex flex-col relative w-full rounded-2xl border-2 border-gray-600/30 hover:border-gray-500/40 focus-within:border-gray-400/50 transition-all duration-300 px-1 glass-effect backdrop-blur-xl shadow-2xl hover:shadow-3xl dark:text-gray-100 group"
			dir={$settings?.chatDirection ?? 'auto'}
			style="background: linear-gradient(135deg, rgba(45, 45, 45, 0.8), rgba(30, 30, 30, 0.9)); box-shadow: 0 8px 32px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.1);"
		>
								{#if files.length > 0}
								...
								<div class=" flex justify-between mt-0.5 mb-2.5 mx-0.5 max-w-full" dir="ltr">
									...
									<div class="self-end flex space-x-1 mr-1 shrink-0">
										...
										{:else if prompt === '' && files.length === 0 && ($_user?.role === 'admin' || ($_user?.permissions?.chat?.call ?? true))}
											<div class=" flex items-center">
												...
												<Tooltip content={$i18n.t('Voice mode')}>
													<button
										class="modern-button rounded-full p-1.5 self-center shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110"
										type="button"
										style="background: linear-gradient(135deg, #667eea, #764ba2); animation: pulseGlow 2s ease-in-out infinite;"
														on:click={async () => {
										...
										class="{!(prompt === '' && files.length === 0)
											? 'modern-button shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110'
											: 'text-white bg-gray-200/50 dark:text-gray-900 dark:bg-gray-700/50 disabled backdrop-blur-sm'} rounded-full p-1.5 self-center"
										type="submit"
										disabled={prompt === '' && files.length === 0}
										style="{!(prompt === '' && files.length === 0) ? 'background: linear-gradient(135deg, #4f46e5, #7c3aed); animation: pulseGlow 2s ease-in-out infinite; box-shadow: 0 4px 15px rgba(79, 70, 229, 0.4);' : ''}"
													>
														<svg
															xmlns="http://www.w3.org/2000/svg"
															viewBox="0 0 16 16"
